...
% figure;
% imagesc(t(1:basicTF.hop:end),tfrsqtic*fs,log1p(abs(SSTx)/5e1)); xlim([0 t(end)]); axis xy ; colormap(1-gray);
...
subplot(2,2,2);
imagesc(tt(1:basicTF.hop:end),tfrsqticTRUE*fs,log1p(abs(VxxTRUE)/5e1)); xlim([0 t(end)]); axis xy ; colormap(1-gray);
xlabel('Time (s)'); ylabel('Frequency (Hz)'); title('STFT on original long signal');
subplot(2,2,3);
imagesc(tt(1:basicTF.hop:end),tfrsqticTRUE*fs,log1p(abs(VxxLSE)/5e1)); xlim([0 t(end)]); axis xy ; colormap(1-gray);
