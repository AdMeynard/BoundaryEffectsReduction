...
\end{equation*}
Let $\bA_0$ denote the deterministic matrix such that $\bA_0 \defeq \bS^{(1)}{\bS^{(0)}}^\inv$. We denote by $\balpha^{(\ell)}_0$ the last row of $\bA_0^\ell$. As a result, for $\ell\in \mathbb{N}^*$, the error vector $\bh^{(\ell)}$ defined by $\bh^{(\ell)}\defeq \balpha^{(\ell)} - \balpha^{(\ell)}_0$ satisfy the equation
\begin{align}
\nonumber
...
In the following paragraphs, we provide some useful results to prove Theorem 1.

\subsubsection{Expression of $\bA_0$}
...
\label{eq:g1g1}
\end{align}
where $C_\bz \defeq 2\left(\sum_{j=1}^J \Omega_j\right)$. Besides, since odd-order moments of a zero-mean multivariate Gaussian random vector are zero, we have:
\begin{align}
\nonumber
...
In this section, we provide bounds on the first and second-order derivatives of $f^{(\ell)}$ at the origin.
...
\right.
\end{align*}
Let $\|\cdot\|_\infty$ denote the uniform norm of a vector or a matrix, \ie~$\|\bM\|_\infty = \max_{n,n'} \bM[n,n']$. Then,
\begin{equation*}
\left\| \left.\dfrac{\partial \tilde\bA}{\partial \bg[r]}\right|_{\bg=\bzero}\right\|_\infty \leq \left(1+\|\bA_0\|_\infty\right)\left\|{\bS^{(0)}}^{-1}\right\|_\infty\ .
\end{equation*}
...
\label{eq:A0}
\end{equation}
and from~\eqref{eq:S0.inv}, we have that $\left\|{\bS^{(0)}}^{-1}\right\|_\infty\leq \dfrac1{\sigma^2}\left(1+\frac{2J}{M}\right)$. Then,
\begin{equation}
\left\| \left.\dfrac{\partial \tilde\bA}{\partial \bg[r]}\right|_{\bg=\bzero}\right\|_\infty \leq \left(1+\max\left(1,\frac{2J}{M}\right)\right)\left(1+\frac{2J}{M}\right)\dfrac{1}{\sigma^2}\ .
\label{eq:dA}
\end{equation}
Besides given expression~\eqref{eq:df}, for all $r\in\{0,\ldots,M(M+1)-1\}$, we have:
\begin{align*}
\left|\left.\dfrac{\partial f_m^{(\ell)}}{\partial \bg[r]}\right\vert_{\bg=\bzero}\right| &\leq 2M\|\bA_0^{\ell-1}\|_\infty \left\| \left.\dfrac{\partial \tilde\bA}{\partial \bg[r]}\right|_{\bg=\bzero}\right\|_\infty + M^2\sum_{\lambda=1}^{\ell-2} \|\bA_0^\lambda\|_\infty \left\| \left.\dfrac{\partial \tilde\bA}{\partial \bg[r]}\right|_{\bg=\bzero}\right\|_\infty  \|\bA_0^{\ell-1-\lambda}\|_\infty \\
&\leq \left(2 + (\ell-2)M\right)M^{\ell-1} \|\bA_0\|_\infty^{\ell-1}\left\|\left.\dfrac{\partial\tilde\bA}{\partial \bg[r]}\right|_{\bg=\bzero}\right\|_\infty\ .
\end{align*}
...
\label{eq:df.bound}
\end{equation}
where $d_{1,\bz,M,\ell}\defeq \left(2 + (\ell-2)M\right)M^{\ell-1}\left(\max\left(1,\frac{2J}{M}\right)\right)^{\ell-1}\left(1+\max\left(1,\frac{2J}{M}\right)\right)\left(1+\frac{2J}{M}\right)$. 
...
\begin{array}{ll}
\left\|{\bS^{(0)}}^{-1}\right\|_\infty^2 &\mbox{if}\quad a_r=1\quad\mbox{or}\quad a_{r'}=1\,, \\
2\left(1+\|\bA_0\|_\infty\right)\left\|{\bS^{(0)}}^{-1}\right\|_\infty^2 &\mbox{else} \\
\end{array}
\right.\\
...
where 
\begin{align*}
d_{2,\bz,M,\ell} &= d_{2,M,\ell}C_{\bz,M}^2\max\left(1,\frac{2J}{M}\right)^{\ell-2} + 2d'_{2,M,\ell}\max\left(1,\frac{2J}{M}\right)^{\ell-1}\left(1+\max\left(1,\frac{2J}{M}\right)\right)\left(1+\frac{2J}{M}\right)^2\ .
\end{align*}
...
\begin{align}
\nonumber
\left| \eta_7[n] \right| &\leq 2\sigma^2\|\balpha_0^{(\ell)}\|_\infty M^3(M+1)\dfrac{d_{1,\bz,M,\ell}}{\sigma^2} \dfrac{2\sigma^2}K + o\left( \dfrac1K\right)\\
...
\begin{align}
\nonumber
\left|\beta_2[n,n']\right| &\leq \sigma^2\left\|\balpha_0^{(\ell)}\right\|_\infty\sum_{m=0}^{M-1}\left|\EE\{\bw[n']\bw_K[m]\}\right| \\
\nonumber
