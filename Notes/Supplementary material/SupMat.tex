...
\end{enumerate}
Condition~\ref{delta.1} is satisfied. Indeed, the differentiation of $f^{(\ell)}$ can be checked as a composition of standard matrix derivation rules.  The detailed calculation of the derivation is not given here for the sake of conciseness. Concerning condition~\ref{delta.2}, by definition of $\bg$, we have:
\[
...
\bg_2 &= \dfrac1K\sum_{k=0}^{K-1}\vec\left(\tilde\bw_k\bw_k^T-\tilde\bI \right) \quad\mbox{where}\quad \tilde\bw_k^T= \begin{pmatrix} \bw_k^T & \bw_{k+1}[M-1] \end{pmatrix} \,.
\end{align*}
First, $\bg_1$ is intrinsically a Gaussian random vector since it is a linear combination of Gaussian random vectors. Second, using the central limit theorem under weak dependence, we can show that $\bg_2$ also converges towards a Gaussian random vector as $K\to\infty$. Combining these two results leads to
\[
\sqrt{K}\ \bg \xrightarrow[K\to\infty]{\cD} \cN(\bzero,\bGamma_0)\ ,
\]
where $\bGamma_0$ is the limit covariance matrix so that $\bGamma_0=\lim\limits_{K\to\infty} K\,\EE\{\bg\bg^T\}$.
Hence, conditions~\ref{delta.1} and~\ref{delta.1} are satisfied. The delta method can therefore be applied; the result is as follows:
\begin{align*}
...
\]
Besides, according to result~\eqref{eq:cov.ap} the first term in~\eqref{eq:gamma.nn}, we thus have
\begin{align*}
