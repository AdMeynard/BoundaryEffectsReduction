...
\]
and $\nu_K$ is the number of non-zero terms in the sum~\eqref{eq:multisum}. Note that $C_T$ is independent of $K$ (but depends on $\bz$ and $\sigma$). The behavior of the order $T$ moment in function of $K$ is therefore only determined by the ratio $\dfrac{\nu_K}{K^T}$.
...
\end{align*}
Consequently, for each value of $k_1$ there exist at least $(K-4)^{T-1}$ combinations of $(k_2,\ldots,k_T)$ such that the correlation term given by $\EE\left\{\prod_{\theta=1}^T \rho_{\theta,k_\theta}(\bw[k_\theta+m'_{r_\theta}],\bw[k_\theta+m_{r_\theta}+a_{r_\theta}]) \right\}$ is zero. Because $T\geq 3$, we develop similar arguments on $k_2$ and $k_3$ to determine other cases where this correlation term vanishes. We subtract these cases to $K^T$, the total number of combinations of $(k_1,\ldots,k_T)$ to obtain the following maximum bound on the number of non-zero terms in the sum~\eqref{eq:multisum}:
\[
