...
\subsubsection{Study of $\bg$}
In the following, for all $r\in\{0,\ldots,M(M+1)-1\}$ we note $\bg[r] = \sigma\bg_1[r] + \sigma^2\bg_2[r]$, where 
\begin{align*}
\bg_1[r] &= \bE_1^{(a_r)}[m_r,m'_r] \\
\bg_2[r] &= \bE_2^{(a_r)}[m_r,m'_r] \ ,
\end{align*}
and $m_r,m'_r, a_r$ are the corresponding coordinates of the matrices associated with $r$ through the vectorization operation~\eqref{eq:vec}. That is why, order-two moments of this random vector is split as follows:
\begin{equation}
...
\nonumber
& \leq \dfrac1{K^2}\left(\sum_{j=1}^J \Omega_j\right)^2 \sum_{k,k'=0}^{K-1} \EE\left\{\bw_k[m'_r]\bw_{k'}[m'_{r'}]\right\} + \EE\left\{\bw_k[m_r+a_r]\bw_{k'}[m'_{r'}]\right\} + \EE\left\{\bw_k[m'_r]\bw_{k'}[m_{r'}+a_{r'}]\right\}  \\
\nonumber
&\hspace{110pt}  + \EE\left\{\bw_k[m_r+a_r]\bw_{k'}[m_{r'}+a_{r'}]\right\} \\
\nonumber
&\leq \dfrac1{K^2}\left(\sum_{j=1}^J \Omega_j\right)^2 \sum_{k=0}^{K-1} 4 \\
&\leq \dfrac{C_\bz^2}{K}\ ,
\label{eq:g1g1}
\end{align}
where $C_\bz = 2\left(\sum_{j=1}^J \Omega_j\right)$. Besides, since odd-order moments of a zero-mean multivariate Gaussian random vector are zero, we have:
\begin{align}
\nonumber
...
\begin{align}
\nonumber
\left|\bE\{\bg_2[r]\bg_2[r']\}\right| &= \dfrac1{K^2}\bigg|\sum_{k,k'=0}^{K-1} \EE\left\{\bw_{k}[m_{r}+a_{r}]\bw_{k}[m'_{r}]\bw_{k'}[m_{r'}+a_{r'}]\bw_{k'}[m'_{r'}]\right\} - \delta_{m_{r}+a_{r},m'_{r}}\bE\{\bg_2[r']\} - \delta_{m_{r'}+a_{r'},m'_{r'}}\bE\{\bg_2[r]\}\\
\nonumber
&\hspace{30pt} + \delta_{m_{r}+a_{r},m'_{r}}\delta_{m_{r'}+a_{r'},m'_{r'}} \bigg| \\
\nonumber
&= \dfrac1{K^2} \bigg| \sum_{k,k'=0}^{K-1} \EE\left\{\bw_{k}[m_{r}+a_{r}]\bw_{k}[m'_{r}]\right\}\EE\left\{\bw_{k'}[m_{r'}+a_{r'}]\bw_{k'}[m'_{r'}]\right\} - \delta_{m_{r}+a_{r},m'_{r}}\delta_{m_{r'}+a_{r'},m'_{r'}} \\
\nonumber
...
\nonumber
&\hspace{30pt}+ \dfrac1{K^2}\sum_{k,k'=0}^{K-1} \EE\left\{\bw_{k}[m_{r}+a_{r}]\bw_{k'}[m'_{r'}]\right\}\EE\left\{\bw_{k}[m'_{r}]\bw_{k'}[m_{r'}+a_{r'}]\right\} \bigg| \\
\nonumber
& \leq \dfrac1{K^2} \sum_{k=0}^{K-1} 2 \\
& \leq \dfrac2{K}\ .
\label{eq:g2g2}
\end{align}
...
\begin{equation}
\left|\bE\{\bg[r]\bg[r']\}\right| \leq \dfrac1K \left( C_\bz^2\sigma^2 + 2 \sigma^4\right)\ .
\label{eq:gg.bound}
\end{equation}
In the same way, we show that higher-order moments behave as $o\left( \dfrac1K \right)$.


\subsubsection{Study of $f^{(\ell)}$}
In this section, we provide bounds of the first and second-order derivatives of $f^{(\ell)}$ at the origin.
...
\right.
\end{align*}
where $\bJ_{m_r,m'_r}\in\RR^{M\times M}$ is the matrix such that $\bJ_{m_r,m'_r}[m,m'] = \delta_{m,m_r}\delta_{m',m'_r}$.
Thus,
\begin{align*}
\left.\dfrac{\partial \tilde\bA}{\partial \bg[r]}\right|_{\bg=0} &= \left\{
\begin{array}{ll}
...
\right.
\end{align*}
Let $\|\cdot\|_\infty$ denotes the uniform norm of a vector or a matrix, \ie $\|\bM\|_\infty = \max_{n,n'} \bM[n,m']$. Then,
\begin{equation*}
\left\| \left.\dfrac{\partial \tilde\bA}{\partial \bg[r]}\right|_{\bg=0}\right\|_\infty \leq \left(1+\|\bA_0\|_\infty\right)\|{\bS^{(0)}}^{-1}\|_\infty\ .
\end{equation*}
Given equation~\eqref{eq:A0.sine}, we have 
\begin{equation}
\|\bA_0\|_\infty\leq \max\left(1,\frac{2J}{M}\right)\ ;
\label{eq:A0}
\end{equation}
and from~\eqref{eq:S0.inv}, we have that $\|{\bS^{(0)}}^{-1}\|_\infty\leq \dfrac1{\sigma^2}\left(1+\frac{2J}{M}\right)$. Then,
\begin{equation}
\left\| \left.\dfrac{\partial \tilde\bA}{\partial \bg[r]}\right|_{\bg=0}\right\|_\infty \leq \dfrac{C_{\bz,M}}{\sigma^2}\ ,
\label{eq:dA}
\end{equation}
where $C_{\bz,M}=\left(1+\max\left(1,\frac{2J}{M}\right)\right)\left(1+\frac{2J}{M}\right)$. Besides given expression~\eqref{eq:df}, for all $r\in\{0,\ldots,M(M+1)-1\}$, we have:
\begin{align*}
\left|\left.\dfrac{\partial f_m^{(\ell)}}{\partial \bg[r]}\right\vert_{\bg=\bzero}\right| &\leq d_{1,M,\ell} \|\bA_0\|_\infty^{\ell-1}\left\|\dfrac{\partial\tilde\bA}{\partial \bg[r]}\right\|_\infty\ .
\end{align*}
where $d_{1,M,\ell}$ is only depending on $M$ and $\ell$.
Therefore, given bounds~\eqref{eq:A0} and~\eqref{eq:dA}, we have:
\begin{equation}
\left|\left.\dfrac{\partial f_m^{(\ell)}}{\partial \bg[r]}\right\vert_{\bg=\bzero}\right| \leq \dfrac{d_{1,\bz,M,\ell}}{\sigma^2}\ ,
\label{eq:df.bound}
\end{equation}
where $d_{1,\bz,M,\ell}=d_{1,M,\ell}\left(1+\frac{2J}{M}\right)\left(\max\left(1,\frac{2J}{M}\right)\right)^{\ell}$. 
...
\begin{align}
\nonumber
\left\| \left.\dfrac{\partial^2 \tilde\bA}{\partial \bg[r]\partial \bg[r']}\right|_{\bg=0}\right\|_\infty &\leq \left\{
\begin{array}{ll}
\|{\bS^{(0)}}^{-1}\|_\infty^2 &\mbox{if}\quad a_r=1\quad\mbox{or}\quad a_{r'}=1\,, \\
2\left(1+\|\bA_0\|_\infty\right)\|{\bS^{(0)}}^{-1}\|_\infty^2 &\mbox{else} \\
\end{array}
\right.\\
...
\begin{align*}
\left|\left.\dfrac{\partial^2 f_m^{(\ell)}}{\partial \bg[r]\partial \bg[r']}\right\vert_{\bg=\bzero}\right| &\leq d_{2,M,\ell} \|\bA_0\|_\infty^{\ell-2}\left\|\dfrac{\partial\tilde\bA}{\partial \bg[r]}\right\|_\infty\left\|\dfrac{\partial\tilde\bA}{\partial \bg[r']}\right\|_\infty + d'_{2,M,\ell}\|\bA_0\|_\infty^{\ell-1}\left\|\dfrac{\partial^2\tilde\bA}{\partial \bg[r]\bg[r']}\right\|_\infty \ ,
\end{align*}
where $d_{2,M,\ell}$ and $d'_{2,M,\ell}$ are only depending on $M$ and $\ell$.
Besides, given results~\eqref{eq:dA} and~\eqref{eq:d2A}, we have:
\begin{equation}
...
where 
\begin{align*}
d_{2,\bz,M,\ell} &= d_{2,M,\ell}C_{\bz,M}^2\max\left(1,\frac{2J}{M}\right)^{\ell-2} + d'_{2,M,\ell}\max\left(1,\frac{2J}{M}\right)^{\ell-1}2\left(1+\max\left(1,\frac{2J}{M}\right)\right)\left(1+\frac{2J}{M}\right)^2\ .
\end{align*}
...
Let us first determine an upper bound on $\left|\bepsilon_1[n]\right|$. We have
\begin{align}
\nonumber
...
\label{eq:A0.sine}
\end{align}
Since $\balpha_0^{(1)}$ is the last row of $\bA_0$, we have
\begin{equation}
...
Let us now determine an upper bound on $\left|\bepsilon_2[\ell]\right|$. Since $\EE\{\bg[r]\}=0$, a second-order Taylor expansion of $\bh^{(\ell)}$ gives
\begin{equation}
...
\label{eq:Eh}
\end{equation}
Thus, given the bounds~\eqref{eq:gg.bound} on $\EE\{\bg[r]\bg[r']\}$ and~\eqref{eq:d2f.bound} on the second derivative of $f_m^{(\ell)}$, we have:
\begin{align}
\nonumber
\left|\bepsilon_2[\ell]\right| &\leq C_\bz\dfrac{M^3(M+1)^2}{4}\left\|\left.\dfrac{\partial^2 f^{(\ell)}}{\partial \bg[r]\partial \bg[r']}\right\vert_{\bg=\bzero}\right\|_\infty \dfrac1K \left( C_\bz^2\sigma^2 + 2 \sigma^4\right) + o\left(\dfrac1K\right)  \\
&\leq \dfrac1K \left(c^{(2)}_1 + \dfrac{c^{(2)}_2}{\sigma^2} \right) + o\left(\dfrac1K\right)\ ,
\label{eq:eps2}
\end{align}
where
\begin{align*}
c^{(2)}_1 &=\dfrac{M^3(M+1)^2}{2}C_\bz d_{2,\bz,M,\ell}\ ,\\
c^{(2)}_2 &=\dfrac{M^3(M+1)^2}{4}C_\bz^3 d_{2,\bz,M,\ell} \ .
\end{align*}
...
\label{eq:eps3}
\end{equation}
where $ c^{(3)} = M^2(M+1)d_{1,\bz,M,\ell}C_\bz$.
...
%\begin{align*}
%\bh^{(\ell)}[m] &= f_m^{(\ell)}(\bg) \\
%& = f_m^{(\ell)}(\bzero) + \sum_{r=0}^{M(M-1)}\left.\dfrac{\partial f_m^{(\ell)}}{\partial \bg[r]}\right\vert_{\bg=\bzero} \bg[r] + \dfrac12\sum_{r,r'=0}^{M(M-1)}\left.\dfrac{\partial^2 f_m^{(\ell)}}{\partial \bg[r]\partial \bg[r']}\right\vert_{\bg=\bzero} \bg[r]\bg[r'] + o\left(\dfrac1K\right)\ .
%\end{align*}
...
First, since $\|\balpha_0^{(\ell}\|^2\leq M\|\balpha_0^{(\ell)}\|_\infty^2\leq M\|\bA_0^{\ell}\|_\infty^2$, we have:
\begin{align}
\nonumber
\eta_1[n] &\leq \sigma^2M\left\|\bA_0^{\ell}\right\|_\infty^2 \\
\nonumber
&\leq\sigma^2M^{\ell}\left\|\bA_0\right\|^{2\ell}_\infty \\
...
Fourth,
\begin{align*}
\left|\eta_4[n]\right| & \leq \sigma C_\bz\|\balpha_0^{(\ell)}\|_\infty \sum_{m,m'=0}^{M-1}\left|\EE\{\bh^{(\ell)}[m]\bw_K[m']\} \right|
\end{align*}
But,
\begin{align}
\nonumber
...
Seventh,
\begin{align*}
\left|\eta_7[n] \right| &\leq 2\sigma^2\|\balpha_0^{(\ell)}\|_\infty \sum_{m,m'=0}^{M-1} \left| \EE\{\bh^{(\ell)}[m]\bw_K[m]\bw_K[m']\} \right| \\
&\leq 2\sigma^2\|\balpha_0^{(\ell)}\|_\infty \sum_{m,m'=0}^{M-1} \sum_{r=0}^{M(M+1)-1}\left|\left.\dfrac{\partial f_m^{(\ell)}}{\partial \bg[r]}\right\vert_{\bg=\bzero}\right| \left| \EE\{\bg^{(\ell)}[r]\bw_K[m]\bw_K[m']\} \right| + o\left( \dfrac1K\right)\ .
\end{align*}
But,
\begin{align*}
...
\begin{align*}
\left|\EE\{\bg_2^{(\ell)}[r]\bw_K[m]\bw_K[m']\}\right| &= \dfrac1K\sum_{k,=0}^{K-1} \EE\left\{\bw_{k}[m_{r}+a_{r}]\bw_{k}[m'_{r}]\bw_K[m]\bw_K[m']\right\} - \delta_{m_r+a_r,m'_r} \EE\left\{\bw_K[m]\bw_K[m']\right\} \\
&\leq \dfrac4K\ .
\end{align*}
Thus,
\begin{align}
\nonumber
\left| \eta_7[n] \right| &\leq 2\sigma^2\|\balpha_0^{(\ell)}\|_\infty M^3(M+1)\dfrac{d_{1,\bz,M,\ell}}{\sigma^2} \dfrac{4\sigma^2}K + o\left( \dfrac1K\right)\\
&\leq 8 M^{\ell+2}(M+1)d_{1,\bz,M,\ell}\left(\max\left(1,\frac{2J}{M}\right)\right)^\ell\dfrac{\sigma^2}{K} + o\left( \dfrac1K\right)\ .
\label{eq:sum7}
\end{align}
...
c_3^{(n)} & =  8 M^{\ell+2}(M+1)d_{1,\bz,M,\ell}\left(\max\left(1,\frac{2J}{M}\right)\right)^\ell\ .
\end{align*}
...
\begin{align}
\nonumber
\beta_2[n,n'] &\leq \sigma^2\left\|\balpha_0^{(\ell)}\right\|_\infty\sum_{m=0}^{M-1}\left|\EE\{\bw[n']\bw_K[m]\}\right| \\
\nonumber
&\leq \sigma^2\left\|\bA_0^{\ell}\right\|_\infty \\
...
\begin{align}
\nonumber
\beta_3[n,n'] &\leq \sigma^2\sum_{m=0}^{M-1}\left|\EE\{\bw[n']\bh^{(\ell)}[m]\bw_K[m]\}\right|\\
\nonumber
...
%Therefore,
%\begin{align*}
%\bgamma[n,n] &\leq \sigma^2\left\|\balpha_0^{(\ell)}\right\|^2-\left(\bepsilon_2[n]+\bepsilon_3[n]\right)^2 + \EE\left\{\left(\bh^{(\ell)}\bz_K\right)^2\right\} + 2\sigma\balpha_0^{(\ell)}\EE\{\bw_K\bh^{(\ell)}\}\bz_K+ 2\sigma\EE\{\bh^{(\ell)}\bw_K\bh^{(\ell)}\}\bz_K \\
%&+\sigma^2\EE\left\{\left(\bh^{(\ell)}\bw_K\right)^2\right\}  + 2\sigma^2\balpha_0^{(\ell)}\EE\{\bw_K\bh^{(\ell)}\bw_K\}
%\end{align*}
