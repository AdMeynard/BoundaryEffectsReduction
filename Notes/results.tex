...
\begin{tabular}{|c||c|c|c|}
  \hline
   ...
   \cline{2-4}
      & STFT & SST & ConceFT\\
   \hhline{|=#=|=|=|}
   {\sf SigExt} & $0.280$ & $0.309$ & $0.367$ \\
   \hline
   Symmetric & $1.168$ & $1.209$ & $1.310$ \\
   \hline
   EDMD & $0.289$ & $0.319$ & $0.375$ \\
   \hline
   GPR & $0.276$ & $0.303$ & $0.361$ \\
   \hline
\end{tabular}
\label{tab:otd.ppg}
\end{table}
...
To evaluate the influence of the noise level on the performance of {\sf BoundEffRed}, we artificially add Gaussian noise to the measured PPG signal. It is thus an additional noise to the measurement noise actually contained in the signal. Fig.~\ref{fig:otd.noise} shows the averaged performance index of {\sf BoundEffRed} for different values of the Signal to Noise Ratio (SNR). This graph illustrates the robustness to noise of {\sf BoundEffRed}. Indeed, whatever the representation considered, the reduction of boundary effects is relatively independent of the noise magnitude.
